...
def create_file(directory: str, existing_files: list[str]) -> str | None:
    ...
    try:
        with open(filepath, "w", encoding="utf-8") as f:
            _ = f.write(generate_content(filepath, version=1))
...
# Return type changed to str | None
def modify_file(_directory: str, existing_files: list[str]) -> str | None:
    ...
    try:
        ...
        with open(filepath_to_modify, "w", encoding="utf-8") as f:
            _ = f.write(generate_content(filepath_to_modify, version=new_version))
...
# Return type changed to str | None
def delete_file(_directory: str, existing_files: list[str]) -> str | None:
    ...
    try:
        ...
        for i in range(NUM_ITERATIONS):
            ...
            if action == "add" and len(current_files) < MAX_FILES:
                ...
                if affected_file_path: 
                    current_files.append(affected_file_path)
                    action_taken = "add"
                ...
                log.info("No file system change.")
                action_taken = "none"

            if action_taken is not None:
                log.info("Action taken: {action_taken}")
        ...
        if base_dir and os.path.exists(base_dir):
            try: 
                ...
                log.info(f"Cleaned up test directory: {base_dir}")
            except Exception as e: 
                log.error(f"Failed to clean up {base_dir}: {e}")

        log.info("--- Test Summary ---")
        log.info(f"Sync Errors: {sync_errors}")
        log.info(f"Index Errors: {index_errors}")
        log.info(f"Search Errors: {search_errors}")
        log.info(f"Validation Failures: {validation_failures}")
        if sync_errors == 0 and index_errors == 0 and search_errors == 0 and validation_failures == 0:
            log.info("Overall Result: PASSED")
        else: 
            log.error("Overall Result: FAILED")
