...
class VectorStore:
    ...
    def _load_model_internal(self, name: str, device_str:str="auto", trust_remote_code:bool=False) -> EmbeddingsModel | None: # Changed Optional
        ...
        if not selected_model:
            raise VectorConfigurationError(f"Model '{name}' is unknown")
    ...
    def generate_embeddings(self, save_every_sec: int = 180, purge: bool = False, model_name_override: str | None = None): # Changed Optional
        ...
        if self.embeddings_matrix is not None and purge is True:
    ...
    def _generate_embeddings_internal(self, save_every_sec: int = 180, purge: bool = False, model_name_override: str | None = None): # Changed Optional
        ...
        if self.current_model is not None:
            batch_size = 8 * self.current_model.get('batch_multiplier')
        else:
            self.log.warning("Batch-multiplier not found for current model")
            batch_size = 32
        for ind, entry in enumerate(self.lib):
            ...
            try:
                ...
                emb_len = 0
                short_title = os.path.splitext(os.path.basename(entry['filename']))[0][:50]
                for chunk_ind in range(0, len(text_chunks), batch_size):
                    print(f"\rEmbedding Progress ({short_title:50s}[{len(text_to_embed)}]): {ind+1}/{total_entries} chunk: {chunk_ind}/{len(text_chunks)}     ", end="", flush=True)
    ...
    def _yellow_line_it_internal(self, text: str, search_embeddings_cpu: torch.Tensor, context_length: int, context_steps: int) -> np.typing.NDArray[np.float32]:
        ...
        try:
            snippet_embeddings: list[torch.Tensor] = self.engine.encode(sentences=clr, show_progress_bar=False, convert_to_numpy=False, batch_ize=128)  # pyright:ignore[reportAssignmentType, reportUnknownMemberType]
